...
import {
    pokemon
} from 'pokemon-go-node-api/pokemons.json';
...
function h(tag, attrs, content) {
    ...
    if (attrs)
        for (var attr in attrs) result.setAttribute(attr, attrs[attr]);
...
var map,
    ...
    clickMarker,
    scanButton,
    currentPosition = [51.511515, -0.117505],
    initZoom = 16;
...
var scanControl = L.Control.extend({
    ...
    onAdd: function() {
        var container = h('div', {
            class: 'leaflet-bar leaflet-control scan-control'
        }, [
            h('span', {
                class: 'name'
            }, 'Scan')
...
function doScan(latlng) {
    ...
    return fetch(`/scan/${socket.id}/${latlng.lat}/${latlng.lng}`)
...
function createPopup(pokemon) {
    ...
    return h('div', {
        ...
        h('span', {
            class: 'name'
        }, pokemonList[pokemon.pokemonid - 1].name),
        counter,
    ]);
}
